...
A standards-based Web App Manifest (src/app/manifest.ts → /manifest.webmanifest)
with start_url /card and display standalone makes "Add to Home Screen" the
primary free quick-access path: the installed icon opens directly onto the
local card and its QR, offline via the existing service worker. Chromium
gets the captured beforeinstallprompt native flow; iOS Safari gets a
concise manual instruction sheet (programmatic install is not possible
there); standalone mode is detected on both. Icons are repository-local
PNGs generated by scripts/generate-icons.mjs — no external services.
Installation adds no server persistence of any kind.
